...
<div class="radar-focus-grid">
  <?php foreach (['hoje' => 'Retornos de hoje', 'amanha' => 'Retornos de amanhã'] as $key => $label): ?>
    ...
    <a class="focus-card <?= $key === 'amanha' ? 'tomorrow' : 'today'; ?>" href="<?= htmlspecialchars(radar_chip_url(['filtro' => $key])); ?>">
      <div>
        ...
        <div class="focus-label"><?= count($bucket) === 1 ? 'cliente para chamar' : 'clientes para chamar'; ?></div>
      </div>
      <?php if ($bucket): ?>
      ...
      <?php else: ?>
        <div class="focus-names"><span>Nenhum retorno previsto</span></div>
      <?php endif; ?>
    </a>
  <?php endforeach; ?>
</div>
